package model 
{
	...
	public class Room 
	{
		...
		public function Init():void 
		{
			...
			InitCurTime()
			m_nLastUserID = GameIF.GetUser().nUserID;
		...
		private function HideNetWeak():void 
		{
			if (GameIF.GetRoom())
			{
				m_vipRoomView.m_boxNetWeak.visible = false;
			}
		}
		...
		{
			m_vipRoomView.m_boxMyGPS.visible = true;
			var gamer:Gamer = GetGamer(GameIF.GetUser().nUserID);
			m_vipRoomView.m_labelMyGPS.text = gamer.sNick + "  " + gamer.sAddress;
		...
		{
			m_vipRoomView.m_boxMyGPS.visible = false;
			
		}
		//显示其他人的位置信息，自己和他的距离
		public function ShowOtherGPS():void
		{
			m_vipRoomView.m_boxOtherGPS.visible = true;
			m_vipRoomView.m_imgOtherGPSBg.visible = true;
		...
		{
			m_vipRoomView.m_boxOtherGPS.visible = false;
			m_vipRoomView.m_imgOtherGPSBg.visible = false;
		}
		//设置两个玩家的距离
		...
		protected function SetCurTime():void 
		{
			...
			m_vipRoomView.m_labCurTime.text = hour.toString() + ":" + minute.toString();
			date = null;
		}
		...
		public function SetGamePlay(gamePlay:*):void 
		{
			...
			vipRoomView.m_labelGamePlay.text = gamePlayName[gamePlay.nType];
			var i:int = 1;
		...
		{
			GetGamer(message.nUserID).OnGetPai(message.getPai);
		...
		{
			GetGamer(GameIF.GetUser().nUserID).OnGuoPaiResponse();
		}
		//吃牌响应
		public function OnDoChiPaiResponse(message:*):void 
		{
			GameIF.GetCanPaiSound(m_sChiSound,GetGamer(GameIF.GetUser().nUserID).sSex);
			GetGamer(GameIF.GetUser().nUserID).OnDoChiPaiResponse(message);
		...
		{
			GameIF.GetCanPaiSound(m_sPengSound,GetGamer(GameIF.GetUser().nUserID).sSex);
			GetGamer(GameIF.GetUser().nUserID).OnDoPengPaiResponse();
			GetGamer(message.nLastID).DelLastputPai(message.lastPai);
			vipRoomView.m_boxCurPutPai.visible = false;
		}
		//碰牌成功通知
		...
		{
			GameIF.GetCanPaiSound(m_sGangSound,GetGamer(GameIF.GetUser().nUserID).sSex);
			var json:JSON = GameIF.GetJson()["GangType"];
			if(message.pai.nGangState==json["AnGang"])
			{
				GetGamer(GameIF.GetUser().nUserID).OnDoAnGangPaiResponse(message);
			}
			else if (message.pai.nGangState==json["MiGang"])
			{
				GetGamer(GameIF.GetUser().nUserID).OnDoGangPaiResponse();
				GetGamer(message.nLastID).DelLastputPai(message.lastPai);
				vipRoomView.m_boxCurPutPai.visible = false;
			}
			else if (message.pai.nGangState==json["MoGang"])
			{
				GetGamer(GameIF.GetUser().nUserID).OnDoMoGangPaiResponse(message);
			}
		...
		{
			GameIF.GetCanPaiSound(m_sTingSound,GetGamer(GameIF.GetUser().nUserID).sSex);
			GetGamer(GameIF.GetUser().nUserID).OnDoTingPaiResponse();
		}
		//听牌成功通知
		...
		{
			GameIF.GetCanPaiSound(m_sTingSound,GetGamer(GameIF.GetUser().nUserID).sSex);
			GetGamer(GameIF.GetUser().nUserID).OnDoQiangTingPaiResponse(message);
			GetGamer(message.nLastID).DelLastputPai(message.lastPai);
			vipRoomView.m_boxCurPutPai.visible = false;
		}
		//抢牌成功通知
		...
		{
			GameIF.GetCanPaiSound(m_sHuSound,GetGamer(GameIF.GetUser().nUserID).sSex);
			GetGamer(GameIF.GetUser().nUserID).OnDoHuPaiResponse();
		}
		//胡牌成功通知
		...
		public function OnGameEndNotify(message:*):void 
		{
			m_nRound += 1;
			vipRoomView.m_boxCurPutPai.visible = false;
			GetGamer(GameIF.GetUser().nUserID).OffCur();
			for each(var gamer:Gamer in m_dalGamer.gamerPool.values)
			{
				//gamer.OnTanPai();
				gamer.OnTanPai();
			...
			//取消托管
			GetGamer(GameIF.GetUser().nUserID).bAI = false;
		}
		//结算
		...
		{
			roomReset();
			GetGamer(GameIF.GetUser().nUserID).OnReady();
		}
		//重置房间
		...
		{
			GetGamer(GameIF.GetUser().nUserID).OnChatResponse(message);
		}
		...
		{
			GetGamer(GameIF.GetUser().nUserID).bAI = message.bDo;
		}

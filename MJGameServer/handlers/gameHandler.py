...
from logic.mudanjiang_room import MuDanJiang_Room
from logic.haerbin_room import  HaErBin_Room
...
class GameHandler(tornado.websocket.WebSocketHandler):
    ...
    def on_create_room(self, message):
        ...
        # 资产更新
        # user.gold = user.gold - cardConfig['gold']
        # user.money = user.money - cardConfig['money']
        # kwargs = {"gold": user.gold, "money": user.money}
        # Dal_User().uqdateUser(user.id, **kwargs)
        # msg = msg_pb2.Msg()
        # msg.type = msg_pb2.EnumMsg.Value('assetupdatenotify')
        # msg.notify.assetUpdateNotify.nUserID = user.id
        # msg.notify.assetUpdateNotify.nGold = user.gold
        # msg.notify.assetUpdateNotify.nMoney = user.money
        # data = msg.SerializeToString()
        # self.write_message(data, True)
        ...
        elif gP.nType == config_game['gameplay']['happy']['id']:

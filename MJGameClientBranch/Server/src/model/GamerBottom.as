package model 
{
	...
	*/
	public class GamerBottom extends Gamer
	{
		...
		protected function RenderWall2():void 
		{
			//手牌
			var paiPoolArr:Array = m_mgrPai.GetPool(m_nPaiPool);
			for (var index:int; index < paiPoolArr.length;index++ )
			{
				var m_item:Gamer0PaiItem = new Gamer0PaiItem;
				var pai:Pai = paiPoolArr[index];
				m_item.pai = pai;
				
				m_item.y = 0;
				m_item.x = (13 - paiPoolArr.length + index) * m_item.paiBg.width;
				vipRoomView.m_listPai.addChild(m_item);
				m_item.on(Event.CLICK, this, onPaiSelected);
			}
		...
		protected function handleCanHuPai():void
		{
			...
			{
				vipRoomView.ShowBaoTip();
			}
			else if (m_message.lastHuType==json['feng'])
			{
				vipRoomView.ShowFengTip();
			}
			if (m_message.bZm)
			{
				vipRoomView.m_btnGuo.offAll();
		...
		protected function GetPaiTweenStop():void 
		{
			...
			var logic:VIPRoomLogic = GameIF.GetLogic(LogicManager.VIPROOMLOGIC) as VIPRoomLogic;

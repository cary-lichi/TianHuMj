...
from logic.CMJHaErBin import CMJHaErBin
from logic.CMJMuDanJiang import CMJMuDanJiang
...
class Gamer:
    ...
    def initGamePlay(self,gamePlay):
        if gamePlay.nType == config_game['gameplay']['haerbin_common']['id'] or \
                        gamePlay.nType == config_game['gameplay']['haerbin_jiahu']['id']:
            self.cmj = CMJHaErBin()
        elif gamePlay.nType == config_game['gameplay']['mudanjiang_common']['id'] or \
                        gamePlay.nType == config_game['gameplay']['mudanjiang_jiahu']['id']:
            self.cmj = CMJMuDanJiang()
...
#计算最终数据
    def calcOverData(self):
        ...
        if self.cmj.CheckWinOp(self.cmj.GetOPByName('bao')):
            self.m_nMoBaoCount = self.m_nMoBaoCount + 1
        if self.m_bZiMo:

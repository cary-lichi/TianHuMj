package blls
{
	...
	public class GamehallLogic extends BaseLogic
	{
		...
		private function InitClickEvent():void
		{
			...
			//创建房间
			m_gamehallView.m_btnCreateHRBRoom.on(Event.CLICK, this, onBtnCreateHRBRoomClicked);
			m_gamehallView.m_btnCreateMDJRoom.on(Event.CLICK, this, onBtnCreateMDJRoomClicked);
			//加入房间
		...
		private function onBtnDeleHRBRoomClicked():void 
		{
			GameIF.DectiveLogic(LogicManager.GAMEHALLLOGIC);
			GameIF.ActiveLogic(LogicManager.VIPHALLLOGIC);
			var logic:VIPHallLogic = GameIF.GetLogic(LogicManager.VIPHALLLOGIC) as VIPHallLogic;
		...
		//创建哈尔滨房间
		private function onBtnCreateHRBRoomClicked():void
		{
			GameIF.DectiveLogic(LogicManager.GAMEHALLLOGIC);
			GameIF.ActiveLogic(LogicManager.VIPHALLLOGIC);
			var logic:VIPHallLogic = GameIF.GetLogic(LogicManager.VIPHALLLOGIC) as VIPHallLogic;
		...
		private function CreateHappyRoom():void
		{
			...
			var createRoomRequestMsg:* = CreateRoomRequest.create({nUserID: GameIF.GetDalManager().daluser.nID, sCardType: "0", gamePlay: createGamePlayMsg, nPlayers: 4});

package blls._Game 
{
	...
	import model.Gamer;
	import model.GamerLeft;
	import model.GamerRight;
	import model.GamerTop;
	import model.Pai;
	import model.Room;
	import model.User;
	...
	{			
		...
		private function HandelMsgInner(msg:*):void 
		{
			...
			/////////// 测试 通知宝牌是什么 正式的还要删掉/////////////
			if (msg.type == 48)//baopaichangenotify
			{
				var baopai:Pai = msg.notify.baopaiChangeNotify.newBao;
				if (baopai)
				{
					OutputLog("当前宝牌","nType=" + baopai.nType+"nValue=" + baopai.nValue);				
				}
				else
				{
					trace("后台发的宝牌为空\r\n");
